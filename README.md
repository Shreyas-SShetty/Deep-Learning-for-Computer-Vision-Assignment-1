...
Before training starts, `train.py` automatically runs the backend build steps equivalent to:

```bash
cd cpp_backend
rm -rf build  # Windows cmd equivalent: rmdir /s /q build
mkdir build
cd build
cmake ..      # plus -A x64 on Windows
